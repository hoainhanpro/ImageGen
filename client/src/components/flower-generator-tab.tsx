...
import { flowerGenerationSchema, type FlowerGenerationRequest } from "@shared/schema";
...
import { Loader2, Flower, AlertCircle, ChevronDown, Code, Image as ImageIcon } from "lucide-react";
...
interface FlowerGeneratorTabProps {}

export function FlowerGeneratorTab({}: FlowerGeneratorTabProps = {}) {
  ...
  const form = useForm<FlowerGenerationRequest>({
    resolver: zodResolver(flowerGenerationSchema),
    defaultValues: {
      templateId: "single-branch",
      variables: {},
      model: "gpt-image-1",
      size: "1024x1024",
      n: 1,
  ...
  const selectedTemplate = FLOWER_TEMPLATES.find(t => t.id === watchedTemplateId);

  // Update processed prompt when template or variables change
  useEffect(() => {
    if (selectedTemplate) {
      const processed = processTemplate(selectedTemplate, watchedVariables || {});
      setProcessedPrompt(processed);
    }
  }, [selectedTemplate, watchedVariables]);
  ...
  }, [watchedTemplateId, form, selectedTemplate]);
  ...
  const generateMutation = useMutation({
    mutationFn: async (data: FlowerGenerationRequest) => {
      try {
        const response = await apiRequest("POST", "/api/flower-generate", data);
    ...
    onSuccess: (data) => {
      if (data.imageUrls && data.imageUrls.length > 0) {
        ...
        toast({
          title: "Success! 🌸",
          description: `Generated ${data.imageUrls.length} flower image${data.imageUrls.length > 1 ? 's' : ''} successfully.`,
        });
      } else {
    ...
    onError: (error: any) => {
      toast({
        ...
        description: error.message || "Failed to generate flower image. Please try again.",
        variant: "destructive",
      });
    },
  });

  const onSubmit = (data: FlowerGenerationRequest) => {
  ...
  const downloadImage = async (url: string, index: number) => {
    try {
      ...
      link.download = `flower-${selectedTemplate?.name.replace(/\s+/g, '-')}-${index + 1}.png`;
  ...
  const getRequestBodyPreview = () => {
    ...
    if (referenceImages.length > 0) requestBody.referenceImages = referenceImages;
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-8">
        ...
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Flower Template</FormLabel>
              ...
              {selectedTemplate && selectedTemplate.variables.map((variable) => (
                <div key={variable.name}>
                  <FormLabel>{variable.placeholder}</FormLabel>
                  <Input
                    placeholder={variable.defaultValue}
                    value={watchedVariables?.[variable.name] || variable.defaultValue}
              ...
              <div>
                <FormLabel>Generated Prompt Preview</FormLabel>
                <Textarea
              ...
              {/* Reference Images from Drive */}
              <div>
                <FormLabel>Reference Images (Tùy chọn)</FormLabel>
                <DriveImageBrowser 
                ...
                {referenceImages.length > 0 && (
                  <div className="mt-2">
                    <p className="text-sm text-slate-600 mb-2">
                      Đã chọn {referenceImages.length} ảnh reference từ Drive
                    </p>
                    <div className="flex flex-wrap gap-2">
                      {referenceImages.map((imageId, index) => (
                        <div key={imageId} className="relative">
                          ...
                          <button
                            type="button"
                            onClick={() => setReferenceImages(prev => prev.filter(id => id !== imageId))}
              ...
              {(watchedOutputFormat === "jpeg" || watchedOutputFormat === "webp") && (
                <FormField
                  control={form.control}
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      />
                      <p className="text-sm text-slate-500">Higher = better quality, lower = smaller file</p>
                      <FormMessage />
                    </FormItem>
                  )}
                />
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    />
                    <p className="text-sm text-slate-500">Để theo dõi và phân tích sử dụng</p>
                    <FormMessage />
                  </FormItem>
                )}
              />
          ...
          {generateMutation.isPending && (
            <div className="mt-6 text-center">
              <Loader2 className="w-8 h-8 border-3 border-green-600 animate-spin mx-auto mb-3" />
              <p className="text-slate-600 dark:text-slate-400">Generating your flower images...</p>
            </div>
          )}
          ...
          {generateMutation.isError && (
            <Alert className="mt-6" variant="destructive">
              ...
              <AlertDescription>
                {generateMutation.error?.message || "Failed to generate flower images. Please try again."}
              </AlertDescription>
            </Alert>
          )}
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">Generated Flower Images</CardTitle>
        </CardHeader>
        <CardContent>
          {generatedImages.length === 0 ? (
            <div className="text-center py-12">
              <Flower className="w-16 h-16 text-slate-300 dark:text-slate-600 mx-auto mb-4" strokeWidth={1} />
              <p className="text-slate-500 dark:text-slate-400">Generated flower images will appear here</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-4">
              {generatedImages.map((url, index) => (
                <ImageResult
                  key={index}
                  src={url}
                  alt={`Generated flower image ${index + 1}`}
